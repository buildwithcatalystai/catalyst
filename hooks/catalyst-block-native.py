...
3. **Native tool guard for the OWNING tab — BUILD-ONLY (2026-07-23).** Native
   ...
   ``_WORKSPACE_BOUND_MODES``. ALLOW_EXACT (``Agent``, ``TodoWrite``,
   native WEB ``WebFetch``/``WebSearch``, and non-FS client tools such as
   ``SendFeedback``) is never touched — **native web access is allowed in every
   mode incl. Build** (read-only network, no FS hazard). The rule of thumb: if a
   tool cannot write a file, it has no reason to be blocked.
...
ALLOW_EXACT = {
    ...
    "SlashCommand",
    "ToolSearch",
    # The native Agent (Task) tool — Claude's & Codex's own sub-tasking. This is the
    # sanctioned way for an agent to fan out / parallelize its work; creating a
    # Mindspace SUBAGENT for that is forbidden (subagents are user-request-only).
    # Plan mode (EnterPlanMode/ExitPlanMode) is NOT here any more (2026-09-06,
    # v0.1.53): it is denied while a Catalyst session is live — see
    # _PLAN_MODE_TOOLS. The PM shapes the PRD in conversation (save_prd) and puts
    # its tasks on the shared ToDos (todos); there is nothing for plan mode to do.
    # NOTE: a spawned native subagent runs under the parent's
    ...
    "WebFetch",
    "WebSearch",
    # Non-filesystem client tools (2026-09-02). The block exists to stop native
    # edits landing on the laptop instead of the workspace — these write no
    # files at all, so blocking them only produced dead ends. SendFeedback in
    # particular was blocked while an agent tried to REPORT a stuck evolve run,
    # and the refusal told it to `abandon_build` — destroying the run it was
    # reporting on.
    "SendFeedback",
    "KillShell",
    "BashOutput",
    # Artifact publishing (2026-09-08, v0.1.57). Reads a LOCAL html file and
    # publishes it to claude.ai — a network publish with no workspace hazard, and
    # the Analyst's natural deliverable (a report page). It fell through to the
    # Build-deny below in deep_analysis and produced the self-contradicting
    # "blocked in Build (mode=deep_analysis)" — see the generalized guard.
    "Artifact",
}
...
# start_app_building. (Agent is native ALLOW_EXACT — never touched by this gate, so it
# stays available in every mode incl. Discover; plan mode is denied in Case 2.)
...
# Claude Code's native plan mode. Denied while this tab has a Catalyst row (any
# mode): the Wingman has ONE planning artefact — the PRD the PM shapes in
# conversation — and one tracker — the shared ToDos. A plan-mode detour
# would park the session behind Claude Code's own approve-the-plan gate instead.
...
_NO_SESSION_BARE = {
    ...
    # open_scratchpad — the `/catalyst` landing call. Takes NO args: the server
    # resolves the caller's ScratchPad itself, and its response is a FULL entry
    # (mode=deep_analysis + sid + app_root + gen_stream_id) that PostToolUse
    # binds this tab to — even if another Mindspace was active (re-running
    # `/catalyst` = go home to the ScratchPad; non-destructive, that Mindspace
    # is one switch_mindspace away).
    "open_scratchpad",
}
...
def _is_allowed_for_owner(tool_name: str) -> bool:
    ...
    native plumbing in ALLOW_EXACT (Agent, TodoWrite, …) pass here. Plan mode is
    denied in Case 2 (_PLAN_MODE_TOOLS).
...
def main() -> int:
    ...
    if is_cat and bare_name not in _NO_SESSION_BARE:
        ...
        return _emit_allow_updated_input(tool_input)
    # ── Plan mode is not part of a Catalyst session (2026-09-06) ────────
    if tool_name in _PLAN_MODE_TOOLS:
        _debug_log(f"  → DENY {tool_name} (plan mode; Catalyst session live)")
        return _emit_deny(
            f"`{tool_name}` is off while a Catalyst session is live — plan mode isn't part "
            "of the Wingman's flow. The PM shapes the PRD in conversation and saves it "
            "with `save_prd`; its tasks become ToDos with `todos`; the "
            "Engineer builds from it. Carry on without plan mode."
        )
    ...
    # Gate on "not an MCP tool", NOT on membership in REDIRECTS (2026-09-08):
    # REDIRECTS only lists the FS/shell names, so every NEWER native tool
    # (Artifact, LSP, Monitor, …) fell past this ALLOW into the Build-deny at the
    # bottom — outside Build — with a reason that literally said "blocked in
    # Build (mode=deep_analysis)". Native means native: open outside Build.
    if not tool_name.startswith("mcp__") and mode not in _WORKSPACE_BOUND_MODES:
    ...
    # (Agent stays always-allowed via ALLOW_EXACT; plan mode is denied above.)
    # External MCP server (any mcp__* that isn't catalyst-mcp — those passed
    # _is_allowed_for_owner above). ONE rule with the native FS/shell tools:
    # blocked only while this tab is in a Build mode (_WORKSPACE_BOUND_MODES),
    # where the app lives on the workspace and every write must go through the
    # coding_workspace surface. Open in menu / Analyst / PM / Curator — and in a
    # tab with no Mindspace bound at all. User decision 2026-09-18 ("even in PM
    # mode all native tools open, only Engineering not"), plugin v0.1.78; 0.1.75/76
    # denied on the row alone and stranded a teammate on 2026-09-17 (Notion after a
    # cleared session), 0.1.77 keyed on the binding.
    if tool_name.startswith("mcp__"):
        bound = bool(((entry or {}).get("session_id") or "").strip())
        if not bound or mode not in _WORKSPACE_BOUND_MODES:
            _debug_log(f"  → ALLOW external MCP {tool_name} (mode={mode}, bound={bound}: not a Build)")
            return 0
        _debug_log(f"  → DENY external MCP {tool_name} (Build mode {mode})")
        return _emit_deny(
            f"`{tool_name}` is an external MCP — blocked while this tab is in a "
            "Build. Do the work with Catalyst's own tools: run_select_query / "
            "run_python / the knowledge base for data, the coding_workspace tools "
            "for the app. Need that MCP for THIS build (a page, a ticket, a source)? "
            "Step out, don't work around: call start_analysis on this same Mindspace "
            "(the bound Mindspace is kept), read as the Analyst, then start_app_building "
            "to resume. Or connect the app in Catalyst (Setup → Tools) and use "
            "external_tools_discover / external_tools_execute here. External MCPs also "
            "reopen after complete_build / abandon_build / `end`."
        )
    ...
    reason = (
        ...
        f"To READ something local or run a local tool right now, step out to the "
        f"Analyst — `start_analysis` on this same Mindspace (the bound Mindspace is "
        f"kept), where every native tool is open — then `start_app_building` to "
        f"resume the Build. Do NOT call `abandon_build` for this: it marks the "
        f"Mindspace abandoned and throws away the build you are in."
    )
