...
- **Plan mode is DENIED while a tab has a Catalyst row (2026-09-06, v0.1.53).** `EnterPlanMode`/`ExitPlanMode` were REMOVED from `ALLOW_EXACT` and are denied in Case 2 via `_PLAN_MODE_TOOLS` (any mode) with a reason that points at `save_prd` + `todos` — the Wingman's one planning artefact is the PM's PRD, its one tracker the shared ledger. No row (no Catalyst session in the tab) → plan mode is untouched.
