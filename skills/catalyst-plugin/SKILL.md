...
- **Three native tools are always allowed for the owning tab during a build:** `TodoWrite`, `AskUserQuestion`, `Skill`, `SlashCommand`, `ToolSearch`, `EnterPlanMode`, `ExitPlanMode` ([catalyst-block-native.py:90](../../../codeGen/catalyst-plugin/hooks/catalyst-block-native.py#L90)). All other native FS/shell tools are denied with a redirect to `coding_workspace__*`. **EXCEPTION — `mode=deep_analysis`:** Case 3 returns `0` (allows) for ALL native tools (Deep Analysis is org-level research with no remote app workspace to protect). The block is otherwise mode-blind — every other live mode (menu/brainstorm/coding/vibe_code) blocks identically. **Because the carve-out allows native Write/Edit/Bash, nothing at the HOOK layer stops an inline web-app build in analysis — the "Analysis is data + scripts + crons, never a web app" rule (added 2026-06-04) is enforced INSTRUCTION-side only** (the `catalyst` skill + the `start_analysis` tool description/return note), deliberately NOT a hook backstop (a hook can't cleanly tell a throwaway script from a web-app file). If you're ever tempted to add such a backstop, that was the considered-and-declined design.
