...
Case 2: this tab's session gate (every tab is "owner" of its own row)
     ...
     tool in ALLOW_EXACT (incl. Agent)? → allow  ·  plan mode (EnterPlanMode/ExitPlanMode)? → deny (2026-09-06)
...
External MCPs (`mcp__*` that aren't catalyst-mcp) are **denied only while the tab is in a Build mode** (`mode ∈ _WORKSPACE_BOUND_MODES`, i.e. coding/vibe_code, with a Mindspace bound) — the same rule as native FS/shell. Open in menu / Analyst (`deep_analysis`) / PM (`spec`) / Curator and in a tab with no Mindspace bound. v0.1.78 (user decision 2026-09-18). Escape from a Build = `complete_build` / `abandon_build` / `end` (per-tab) / `switch_mindspace` with no target — never `current_session`, which only reads. History: 0.1.75/76 denied on the row alone (stranded a cleared tab, Notion, 2026-09-17); 0.1.77 keyed on the binding. **Sanctioned step-out (v0.1.79):** an Engineer needing an external MCP mid-Build calls `start_analysis` on the same Mindspace (the hook stamps the bound id — same Mindspace, mode flips to `deep_analysis`), reads, then `start_app_building` back; the Build deny message says so. A file that must land in the build goes through the transfer curl (open in every mode), not a mode change; reading local material is a step-out like any other (the native redirect message already says so — and says never `abandon_build` for it).
