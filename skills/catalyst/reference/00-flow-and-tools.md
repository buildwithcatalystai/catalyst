...
| `start_spec(prompt?)` | **the PM** | Shape build-ready PRDs *with the user*, in conversation. Author each PRD, write it with `save_prd` (many accumulate in the PRD folder); **each PRD names its builder** — the company's **HUMAN engineers** (the PM then owns the delivery: push the PRD's tasks to their project-management tool, **Mindspace id on every ticket**) or the **Hive's Engineer**. **Acts on your CURRENT Mindspace.** **Denied:** the build actions (`write`/`edit`/`bash`/`playwright`); `save_prd` IS allowed. |
...
- **The PM** — the same investigation surface **plus `save_prd`** to author build-ready PRDs, and the external-tools flow to push a PRD's tasks to the user's PM tool when their human engineers build it (Mindspace id on every ticket). **Denied:** the build actions (`write` / `edit` / `bash` / `playwright_test`).
