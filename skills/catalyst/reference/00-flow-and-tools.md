...
| Tool | Purpose |
|---|---|
| `open_scratchpad` | **The landing call — your first and only call on every `/catalyst` activation.** Auth probe + opens the caller's ScratchPad with the Analyst live, one hop, no args. `authenticated:false` → read `message`, do it, stop; `status:error` → read `fix_required`; ok → greet (+ their ToDos' one line from `digest`) + "What's on your mind?"; relay `todos_block`. Built lean — auth, the ScratchPad, their ToDos, nothing else; a Mindspace the user NAMES is resolved with a silent `list_mindspaces`. |
